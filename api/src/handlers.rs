...
use ssi::{
    ...
    did_resolve::get_verification_methods,
    jsonld::ContextLoader,
    jwk::{Algorithm, JWK},
    jws::{decode_jws_parts, split_jws, DecodedJWS},
    jwt::{decode_unverified, decode_verify, encode_sign},
...
pub async fn id_token(id: Uuid, db: &mut dyn DBClient) -> Result<String, CustomError> {
    // let res = RegistrationMetadata::new();

    // let jwk = did_ion::DIDION::generate_key().context("Could not generate key")?;
    let mut jwk: JWK =
        serde_json::from_str(DID_JWK).map_err(|e| anyhow!("Could not load JWK: {}", e))?;
...
// impl TryFrom<ResponseRequestBase64> for ResponseRequest {
//     type Error = CustomError;
//     fn try_from(value: ResponseRequestBase64) -> Result<Self, Self::Error> {
//         Self {
//             id_token: base64::decode(id_token)
//         }
//         if value <= 0 {
...
pub async fn response(
    ...
    let did_resolvers = did_resolvers();
    ...
    let id_token: IdTokenSIOP = decode_unverified(&params.id_token)
        .map_err(|e| CustomError::BadRequest(format!("id_token decoding failed: {}", e)))?;
    println!("{:?}", id_token);

    let (header_b64, payload_enc, signature_b64) =
        split_jws(&params.id_token).context("Could not split JWS")?;
    let DecodedJWS {
        header,
        signing_input,
        payload,
        ...
        .context("Could not decode JWS")?;
    let claims: IdTokenSIOP =
        serde_json::from_slice(&payload).context("Could not extract SIOP ID Token claims")?;
    if claims.iss != "https://self-issued.me/v2/openid-vc" {
        // || claims.sub != vc.id {
        return Ok(false);
    }
    ...
    .await
    .context("Could not resolve sub")?;
    if !vms.iter().any(|(_, vm)| {
        if let Some(jwk) = &vm.public_key_jwk {
            decode_verify::<IdTokenSIOP>(&params.id_token, jwk).is_ok()
        } else {
            false
        }
    }) {
...
pub(crate) mod tests {
    ...
    async fn signed_request() {
        let jwt = id_token(Uuid::new_v4(), &mut MemoryDBClient::new())
            .await
            .unwrap();
