...
pub async fn prepare_openid4vp_mdl_response(request: RequestObject, session_transcript: Tag24<SessionTranscript>, documents: Documents) -> Result<PreparedDeviceResponse, Error>{
    let unattended_session_manager = UnattendedSessionManager::new(session_transcript, documents).unwrap();
    Ok(unattended_session_manager.prepare_mdl_response(request).unwrap())
}

pub async fn complete_openid4vp_mdl_response<S, Sig>(prepared_device_response: PreparedDeviceResponse, signing_key: S) -> Result<Jarm, Error>  where
S: Signer<Sig>,
